...
class PosteriorSampler(object):
    ...
    def __init__(self, ensemble, nreplicas=1, freq_write_traj=100., freq_save_traj=100., verbose=False):
...
#        np.save('compiled_nuisance_parameters.npy',self.nuisance_para)
    ...
    def get_model(self):
        ...
        model = [[] for i in range(self.nstates)]
        for s in range(self.nstates):
            for R in self.ensemble[s]:
                model[s].append([R.restraints[j]["model"] for j in range(R.n)])
        return model


    def get_SSE(self, states):
        ...
        if self.nreplicas > 1:
            ...
            for k,R in enumerate(self.ensemble[0]): # loop through Restraints
                ...
                avg_r = np.mean([self.model[state][k] for state in states], axis=0)
                # Create list of dictionaries that matches Restraints
                f = [dict(model=avg_r[i], exp=R.restraints[i]["exp"],
                    weight=R.restraints[i]["weight"]) for i in range(len(avg_r))]
                all_sse.append(R.compute_sse(f))
        else:
            all_sse = np.zeros(len(self.ensemble[int(states[0])]))
        return all_sse
